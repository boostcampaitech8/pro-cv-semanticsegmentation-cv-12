...
class SAM2UNet(nn.Module):
    def __init__(self, checkpoint_path=None) -> None:
        ...
        self.edge_head = nn.Conv2d(64, 29, kernel_size=1)
...
if __name__ == "__main__":
    try: 
        model = SAM2UNet(checkpoint_path="sam2_hiera_large.pt").cuda()
    except:
        print("체크포인트 경로를 확인해주세요. 테스트를 위해 체크포인트 없이 로드합니다.")
        # 체크포인트가 없으면 가중치 로드 없이 구조만 생성 (테스트용)
        model = SAM2UNet().cuda()
    ...
    with torch.no_grad():
        ...
        out, out1, out2, out_edge = model(x)
        print(f"Input Shape: {x.shape}")
        print(f"Mask Shape : {out.shape}")      # [1, 29, 352, 352]
        print(f"DS1 Shape  : {out1.shape}")     # [1, 29, 352, 352]
        print(f"DS2 Shape  : {out2.shape}")     # [1, 29, 352, 352]
        print(f"Edge Shape : {out_edge.shape}") # [1, 29, 352, 352]